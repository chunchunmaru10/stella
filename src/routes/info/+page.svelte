...
<section class="prose min-w-full pb-8 dark:prose-invert prose-h2:mt-6 prose-img:rounded-lg">
	...
	In settings, you can toggle to display the potential ratings instead of the actual ratings (default).
	A potential rating only applies to relics that only has 3 substats, which assumes that the fourth substats
	is always the best substats a relic can get (taking into account the sub and main stats that have already
	been obtained). This can help you to decide whether a relic is worth leveling up or not when it is
	still at level 0.
