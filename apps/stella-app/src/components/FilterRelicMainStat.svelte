<script lang="ts">
	...
	export let mainStats: Map<string, string>;
	export let selectedMainStats: Map<string, string>;

	function selectMainStat([name, image]: [string, string]) {
		let existing = selectedMainStats.get(name);

		if (!existing) selectedMainStats.set(name, image);
		else selectedMainStats.delete(name);
...
<div class="mt-6 flex items-center gap-4">
	<h3 class="text-lg font-semibold">Main Stat</h3>
...
<Modal let:dialog>
	...
	<div class="max-h-full">
		<ul
			class="grid h-[500px] max-h-full grid-cols-1 gap-2 overflow-y-auto px-2 scrollbar-thin md:grid-cols-2"
		>
			{#each mainStats as [name, image]}
				<li class="w-full">
					<button
						...
						on:click={() => selectMainStat([name, image])}
					>
						<Checkbox disabled checked={!!selectedMainStats.get(name)} class="ml-2" />
